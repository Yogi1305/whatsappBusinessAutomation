...
const Models = () => {
    ...
    useEffect(() => {
        const fetchModels = async () => {
            try {
                const response = await axiosInstance.get('http://localhost:8000/dynamic-models/');
    ...
    const handleModelSelect = async (model) => {
        ...
        setLoading(true);
        try {
            const response = await axiosInstance.get(`http://localhost:8000/dynamic-model-data/${model.model_name}/`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axiosInstance.post(`http://localhost:8000/dynamic-model-data/${selectedModel.model_name}/`, formValues, {
                headers: {
            ...
            setShowModal(false);
            // Refresh model data
            const updatedData = await axiosInstance.get(`http://localhost:8000/dynamic-model-data/${selectedModel.model_name}/`);
